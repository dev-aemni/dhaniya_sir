...
async function handleSlash(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (name === '8ball') {
    ...
    const answers = [
      'Yes.', 'No.', 'Maybe.', 'Definitely.', 'Not likely.', 'Ask again later.', 'It is certain.', 'Very doubtful.'
    ];
...
client.on(Events.InteractionCreate, async interaction => {
  try {
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('giveaway_create:')) {
      ...
      const messageId = sent?.id as string | undefined;
      if (!messageId) {
...
client.on(Events.MessageCreate, async (message: Message) => {
  ...
  if (command === 'purge' || command === 'clear') {
    ...
    if (result.error) {
      const replyMsg = await sendToChannel(message.channel, result.error) as Message | null;
      if (replyMsg && typeof replyMsg.delete === 'function') setTimeout(() => replyMsg.delete().catch(() => null), 5000);
    } else {
      const msgStr = `Successfully deleted **${result.deleted}** message(s)${targetChannel.id !== message.channelId ? ` in <#${targetChannel.id}>` : ''}.`;
      const replyMsg = await sendToChannel(message.channel, msgStr) as Message | null;
      if (replyMsg && typeof replyMsg.delete === 'function') setTimeout(() => replyMsg.delete().catch(() => null), 5000);
    }
    return;
  }
